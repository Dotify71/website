...
:::note
Si estás usando una versión antigua de Lefthook y ves secuencias de escape ANSI sin procesar en tu terminal, deberías actualizar Lefthook o configurar los colores de Lefthook / establecer `NO_COLOR`. Alternativamente, puedes pasar `--colors=off` a Biome.
:::
