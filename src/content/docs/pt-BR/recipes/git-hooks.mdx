...
:::note
Se você estiver usando uma versão mais antiga do Lefthook e vir sequências de escape ANSI não processadas em seu terminal, você deve atualizar o Lefthook ou configurar as cores do Lefthook / definir `NO_COLOR`. Como alternativa, você pode passar `--colors=off` para o Biome.
:::
