...
:::note
Si vous utilisez une ancienne version de Lefthook et voyez des séquences d'échappement ANSI brutes dans votre terminal, vous devriez soit mettre à jour Lefthook, soit configurer les couleurs de Lefthook / définir `NO_COLOR`. Alternativement, vous pouvez passer `--colors=off` à Biome.
:::
